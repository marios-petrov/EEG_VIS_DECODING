...
def train_epoch(
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        with torch.cuda.amp.autocast(dtype=torch.float16):
            ...
            # 3. Diffusion alignment loss (optional, controlled by --diffusion-weight)
            ...
            if args.diffusion_weight > 0:
                ...
                with torch.no_grad():  # Transformer is frozen, no gradients needed
...
def main():
    ...
    parser.add_argument('--diffusion-weight', type=float, default=0.0)  # Default 0 = disabled
    ...
    if rank == 0:
        ...
        if args.diffusion_weight > 0:
            print(f"  ⚠️  DIFFUSION LOSS ENABLED - Will use SD 3.5 transformer (high VRAM!)")
        else:
            print(f"  ✓ Diffusion loss disabled - Only reconstruction loss")
