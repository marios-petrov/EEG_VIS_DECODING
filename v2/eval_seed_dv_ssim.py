...
def main():
    ...
    for sample in tqdm(samples[:max_samples]):
        ...
        # Stage1 saves GT as folder: gt_frames/00000/frame_000.png, frame_001.png, ...
        gt_path = None
        
        # Try folder structure first (from stage1)
        gt_folder = gt_frames_dir / f"{idx:05d}"
        if gt_folder.exists() and gt_folder.is_dir():
            # Use middle frame (frame_003 of 6 frames, or first available)
            middle_frame = gt_folder / "frame_003.png"
            if middle_frame.exists():
                gt_path = middle_frame
            else:
                # Find any frame
                frame_files = sorted(gt_folder.glob("frame_*.png"))
                if frame_files:
                    gt_path = frame_files[len(frame_files)//2]  # middle frame
        
        # Fallback to single PNG
        if gt_path is None:
            single_gt = gt_frames_dir / f"{idx:05d}.png"
            if single_gt.exists():
                gt_path = single_gt
            else:
                single_gt = gen_dir / "gt" / f"{idx:05d}.png"
                if single_gt.exists():
                    gt_path = single_gt
