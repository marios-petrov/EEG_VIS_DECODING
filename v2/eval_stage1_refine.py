...
# SEED-DV Concept names (1-indexed: 1-40)
# Correct mapping from paper Figure 1(A)
SEED_DV_CONCEPTS = {
...
class SEEDDVDataset(Dataset):
    ...
    def __init__(
        ...
        # Load video labels to fix concept_id mapping
        # Old preprocessing stored position (0-39) as concept_id
        # Actual concept_id should come from All_video_label.npy
        self.video_labels = None
        if video_labels_path and Path(video_labels_path).exists():
            self.video_labels = np.load(video_labels_path)  # (7, 40)
            print(f"  Loaded video labels for concept_id correction: {self.video_labels.shape}")
        else:
            # Try default location
            default_path = Path("/local-scratch/SEED/Video/meta-info/All_video_label.npy")
            if default_path.exists():
                self.video_labels = np.load(default_path)
                print(f"  Loaded video labels from default location: {self.video_labels.shape}")
        ...
        for subj_id in subject_ids:
            ...
            for seg in metadata['segments']:
                if not seg['is_train']:
                    ...
                    # Fix concept_id if we have video_labels
                    # Old preprocessing: concept_id = position in block (0-39)
                    # Correct: concept_id = All_video_label[block_id, position] (1-40)
                    if self.video_labels is not None:
                        block_id = seg['block_id']
                        position = seg['concept_id']  # This was actually position
        ...
        # Show sample mapping
        if self.samples and self.video_labels is not None:
            print(f"  Sample concept_id check (first 3):")
            for s in self.samples[:3]:
                cid = s['concept_id']
                name = get_concept_name(cid)
                print(f"    block={s['block_id']}, concept_id={cid} → '{name}'")
...
def main():
    ...
    parser.add_argument('--video-labels', type=str, 
                       ...
                       help='Path to All_video_label.npy for correct concept_id mapping')
